...
1. Look in `.tmp/video_clipper/` for the most recently modified subdirectory
...
Read the scoring criteria from `workflows/video_clipper/clipping_agent_skills.md` and apply them.
...
1. Write the clips JSON to `.tmp/video_clipper/<video>/` as `<video_name>_clips.json`
2. Follow the exact schema defined in `workflows/video_clipper/clipping_agent_skills.md`
...
Clips saved. Next steps:
  ...
  .venv/bin/python workflows/video_clipper/video_clipper.py "<drive_url>" --draft

  # Once approved, cut all + upload
  .venv/bin/python workflows/video_clipper/video_clipper.py "<drive_url>" --cut-and-upload
```
